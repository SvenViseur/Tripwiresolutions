...
def cli = new CliBuilder( usage: 'civl_read_and_remove.groovy ' )
cli.with{
        h longOpt: 'help', 'Show usage information'
        p longOpt: 'project', args:1, required: true, 'Project Name'
        t longOpt: 'workreptype', args:1, required: true, 'Project Type'
        f longOpt: 'file', args:1, required: true, 'File Name'
}
...
def remove_scenario( String scenario_name, String scenario_globalId, OdiInstance odiInstance ) {
   ...
   scen_found=((IOdiScenarioFinder)tme.getFinder(OdiScenario.class)).findByGlobalId( scenario_globalId )
   ...
   if ( scen_found != null ){
     try {
        ...
        tme.remove( scen_found );
        tm.commit( trans_scen );
        println "   ok"
     }
        catch( e ) {
        println "   not removed"
        tm.rollback( trans_scen )
        println ( e );
     }
   } else
   {
