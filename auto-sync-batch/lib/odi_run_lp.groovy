...
cli.with{
        ...
        i longOpt: 'input_param', args:1, required: false, 'Input parameter file'
}
...
if (options.p) {
   pMapList = []
   pMapList.add( options.p.split('#'))
}

if (options.i) {
   def File fileparam = new File( options.i );
   pParamList_tmp = ( fileparam as List ).collect { it.split( ':' ) }
   pParamList=pParamList_tmp.findAll { item -> item.size()>1 }
}

Map pParameters = new HashMap();

pParamList.each() { list_item ->
   pParameters.put( list_item[0], list_item[1] )
}
...
//      def parms = new StartupParams(props);
      ...
      def parms = new StartupParams(pParameters);
      ...
      try{
         ...
         if (exeInfo.getSessionStatus() == SessionStatus.ERROR) {
            println "ERROR: Status => "+exeInfo.getSessionStatus()
            println "ERROR: Message => "+exeInfo.getStatusMessage()
            println "ERROR: StatusInRepo =>"+exeInfo.getSessionStatusInRepository()
            System.exit(1)
         }
      }
      catch (e) {
...
//      LoadPlanStartupParams startupParams = new LoadPlanStartupParams(paramsValues);
      ...
      LoadPlanStartupParams startupParams = new LoadPlanStartupParams(pParameters);
               ...
               lpStatInfo.each() { lpstat ->
                  if (lpstat.getLoadPlanRunStatus() != 'R'){
                     if ( lpstat.getLoadPlanRunStatus() == "E") {
                        println "ERROR: RunMessage => "+lpstat.getLoadPlanRunMessage()
                        println "ERROR: RunCode => "+lpstat.getLoadPlanRunRC()
                        System.exit(1)
                     }
                     still_waiting = false
                  }
