...
ScriptName="tomcatpasswordprep.sh"
...
then
  LogLineWARN "Ticket contains file deletions. This is currently NOT supported."
fi
...
if [ "$TheTicketType" != "password" ]; then
  echo "ERROR: This ticket is not a tomcat type config ticket. The files attached to the ticket do not match the expected "
