#!/bin/bash
#### delphiserverconfigprep.sh script
...
#################################################################
ScriptName="delphiserverconfigprep.sh"
ScriptVersion="1.0.1"
...
ActionType="delphiserver-config-prep"
MaakTmpTicketFolder
cd ${TmpTicketFolder}
...
if [ ${TheTicketType} != "config" ]; then
  echo "Error: This ticket is not a delphiserver config-type. The files attached to the ticket do not match the expected "
