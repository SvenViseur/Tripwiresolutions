...
# dexa # 23/11/2017  # corr. location of delete script             #        #
# dexa # 23/11/2017  # added script version info                   # 1.0.0  #
# lekri # 12/11/2021 # ondersteuning ACC & SIM TraceIT/4me         # 1.0.1  #
#      #             #                                             #        #
#############################################################################
#

ScriptName="jbosswarinst.sh"
ScriptVersion="1.0.1"
...
cat >${TmpCmd2File} << EOL
#!/bin/bash
## sudo /bin/su - ${TheUsr} -c "cp -r ${TargetFolder}/from_nfs/* ${TargetFolder}/.."
...
if [ -e "deployIT-scripts/dodelete.sh" ]; then
  ...
  sudo /bin/su - ${TheUsr} -c "${TargetFolder}/from_nfs/deployIT-scripts/dodelete.sh"
fi
EOL
chmod +x ${TmpCmd2File}
